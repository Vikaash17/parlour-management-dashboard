...
export function Services() {
  ...
  async function onSubmit(data: ServiceForm) {
    try {
      setSaving(true)
      const payload = { name: data.name, category: data.category, price: Number(data.price) }
      if (editing) {
            ...
            <Card key={service.id}>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <span className={`inline-block text-xs px-2 py-0.5 rounded-full mt-1 ${catColors[service.category] || 'bg-gray-100'}`}>
                    {service.category}
                  </span>
